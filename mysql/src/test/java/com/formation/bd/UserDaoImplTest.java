...
public class UserDaoImplTest {
	...
	private int getMaxId() throws SQLException {
		...
		var rs = stmt.executeQuery("select max(id) as id from USERTEST");
		rs.next();
	...
	private List<User> getUsersInRange(int minId, int maxId) throws SQLException{
		...
		var stmt = conn.prepareStatement("select id, name from USERTEST where id >= ? and id <= ?");
		stmt.setInt(1, minId);
		stmt.setInt(2, maxId);
	...
	public void testSave() throws SQLException {
		...
		var stmt = conn.createStatement();
		var rs = stmt.executeQuery("select id, name from USERTEST order by id desc");
		var result = rs.next();
